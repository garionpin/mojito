...
public class PushRunService {
    ...
    /**
     * Retrieves the list of TextUnits and their corresponding Assets, to allow
     * the client to further filter down the list on various Asset criteria.
     */
    public List<TmTextUnitWithAsset> getPushRunTextUnits(PushRun pushRun) {
        List<PushRunAsset> pushRunAssets = pushRunAssetRepository.findByPushRun(pushRun);

        return pushRunAssets.stream()
                .map(pushRunAsset -> {
                    List<PushRunAssetTmTextUnit> assetTmTextUnits =
                            pushRunAssetTmTextUnitRepository.findByPushRunAsset(pushRunAsset);

                    return assetTmTextUnits.stream()
                            .map(pushRunAssetTmTextUnit ->
                                         new TmTextUnitWithAsset(pushRunAssetTmTextUnit.getTmTextUnit(),
                                                                 pushRunAsset.getAsset()))
                            .collect(Collectors.toList());
                })
