...
public interface TMTextUnitStatisticRepository  extends JpaRepository<TMTextUnitStatistic, Long> {
    // @Query("select tus from TMTextUnitStatistic tus where tus.tmTextUnit.asset.id = :assetId")
    // @Query("select tus from TMTextUnitStatistic tus join fetch tus.tmTextUnit tmTextUnit join fetch tus.tmTextUnit.asset asset where tus.tmTextUnit.asset.id = :assetId")
